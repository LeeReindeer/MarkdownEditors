...
package moe.leer.markdowneditor.widget;
...
public class TabView extends HorizontalScrollView {
  ...
  public void addTab(String title, OnClickListener onClickListener) {
    ...
    if (mLayout.getChildCount() <= 0) {
      ...
      //设置前一个的字体颜色
      TextView lastTitle = mLayout.getChildAt(mLayout.getChildCount() - 1).findViewById(R.id.file_name);
  ...
  public boolean removeTab() {
    ...
    if (count > 1) {
      ...
      //设置最后一个的字体颜色为白色
      TextView lastTitle = lastView.findViewById(R.id.file_name);
